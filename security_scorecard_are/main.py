...
class SecurityScoreCard(PluginBase):
    ...
    def api_call_helper(self, url, method, data=None):
        ...
        for attempt in range(MAX_RETRY_COUNT):
            ...
            if resp.status_code == 200 or resp.status_code == 201:
                ...
                break
            elif resp.status_code in [429, 503, 301] and attempt < (MAX_RETRY_COUNT - 1):
    ...
    def validate(self, data):
        ...
        if "portfolio_name" not in data or not data["portfolio_name"] or type(data["portfolio_name"]) != str:
            self.logger.error(
                "SecurityScoreCard Plugin: Validation error occured Error: Invalid SecurityScoreCard  portfolio name provided."
            )
        ...
        if "api_token" not in data or not data["api_token"] or type(data["api_token"]) != str:
            self.logger.error(
                "SecurityScoreCard Plugin: Validation error occured Error: Invalid api_token provided."
            )
            return ValidationResult(
                success=False,
    ...
    def check_result(self, record_value, application_value):
        if isinstance(application_value, list):
            return record_value in application_value
    ...
    def push(self, applications, mapping):
        ...
        self.logger.info("Plugin SecurityScorecard: Sending the applications to Security Scorecard.")
